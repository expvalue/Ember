"""
firebot_v5.py — Fire-detection bot with extinguisher trigger

Fire model only (no general objects) for max FPS.
Drives forward → stops on fire → 1.3s of fire → sends 'E' →
Arduino fires servo 1, waits 750ms, fires servo 2 automatically.

Arduino sketch: pbot_v5.ino

Usage:
    sudo chmod 666 /dev/ttyACM0
    python3 firebot_v5.py
"""
...
ARDUINO_PORT = os.environ.get("ARDUINO_PORT", "/dev/ttyACM0").strip()
ARDUINO_BAUD = 9600
...
FIRE_CONF       = 0.35
YOLO_IMG_SIZE   = 320         # smaller = faster
TRIGGER_HOLD_SEC = 1.3        # fire must be seen for this long before triggering
GRACE_SEC        = 0.5        # detection can flicker without resetting timer


# ══════════════════════════════════════════════════════════════════════════════
#  ARDUINO SERIAL (with auto-reconnect)
# ══════════════════════════════════════════════════════════════════════════════
...
def send_cmd(cmd: str):
    global arduino, last_reconnect_attempt
    if cmd not in ("F", "S", "E"):
        return
    if arduino is None:
        now = time.monotonic()
    ...
    try:
        ...
        print(f"[firebot] Serial write failed: {e} — will reconnect", flush=True)
        try:
            arduino.close()
        except Exception:
            pass
...
print("\n[firebot] RUNNING (fire model only, 320px)", flush=True)
print(f"[firebot]   Extinguish after {TRIGGER_HOLD_SEC}s of fire", flush=True)
...
try:
    while True:
        ...
        # ── Run fire model only ───────────────────────────────────────────
        fire_results = fire_model(frame, verbose=False, imgsz=YOLO_IMG_SIZE)
        ...
        if not extinguished and fire_active and fire_start is not None:
            elapsed = now - fire_start

            if elapsed >= TRIGGER_HOLD_SEC:
                extinguished = True
                send_cmd("E")
                print("[firebot] >>> EXTINGUISH COMMAND SENT! Arduino handles both servos.", flush=True)
            else:
                remaining = TRIGGER_HOLD_SEC - elapsed
                cv2.putText(frame, f"EXTINGUISH IN: {remaining:.1f}s", (10, fh-80),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
        elif not extinguished:
            if fire_start is not None and not fire_active:
        ...
        if now - fps_time >= 1.0:
            ...
            fps_count = 0
            fps_time = now
        cv2.putText(frame, f"FPS: {fps_display:.1f}", (fw-140, 30),
