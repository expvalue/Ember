/*
 * pbot_v4.ino — Fire bot with staggered extinguisher servos
 * Arduino Mega  •  9600 baud  •  Osoyoo Mecanum
 *
 * Commands from firebot_v4.py:
 *   'F' → drive forward
 *   'S' → stop all motors
 *   '1' → fire servo 1
 *   '2' → detach servo 1 (free up power) then fire servo 2
 *
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println(F("pbot v4 ready. F=fwd S=stop 1=servo1 2=servo2"));
  lastCmdTime = millis();
}
...
void loop() {
  while (Serial.available()) {
    ...
    if (c == '2') {
      servo1.detach();    // stop holding servo 1 — frees power for servo 2
      delay(100);         // let power stabilize
      servo2.write(180);
      Serial.println(F(">> SERVO 2 FIRED (servo 1 released)"));
    }
  }
