/*
 * pbot_v3.ino — Fire bot with staggered extinguisher servos
 * Arduino Mega  •  9600 baud  •  Osoyoo Mecanum
 *
 * Commands from firebot_v3.py:
 *   'F' → drive forward (all 4 wheels)
 *   'S' → stop all motors
 *   '1' → trigger servo 1 (does NOT stop motors — Python handles that)
 *   '2' → trigger servo 2
 *
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println(F("pbot v3 ready. F=fwd S=stop 1=servo1 2=servo2"));
  lastCmdTime = millis();
}
...
void loop() {
  while (Serial.available()) {
    ...
    if (c == 'S') { stop_all(); }
    if (c == '1') { servo1.write(180); Serial.println(F(">> SERVO 1")); }
    if (c == '2') { servo2.write(180); Serial.println(F(">> SERVO 2")); }
  }
